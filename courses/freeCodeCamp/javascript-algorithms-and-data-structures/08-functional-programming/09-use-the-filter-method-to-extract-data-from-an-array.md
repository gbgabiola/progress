# [Functional Programming: Use the filter Method to Extract Data from an Array](https://learn.freecodecamp.org/javascript-algorithms-and-data-structures/functional-programming/use-the-filter-method-to-extract-data-from-an-array)
...
## Solution 1
```js
// Using map first then filter
// let filteredList = watchList.map((item) => ({ title: item.Title, rating: item.imdbRating })).filter(item => Number(item.rating) >= 8.0);
let filteredList = watchList.map(function(item) {
...
## Solution 2
```js
// Using filter first then map
// let filteredList = watchList.filter((item) => Number(item.imdbRating) >= 8.0).map((item) => ({title: item.Title, rating: item.imdbRating}));
