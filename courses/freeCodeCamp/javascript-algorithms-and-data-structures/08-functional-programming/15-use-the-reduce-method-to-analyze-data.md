# [Functional Programming: Use the reduce Method to Analyze Data](https://learn.freecodecamp.org/javascript-algorithms-and-data-structures/functional-programming/use-the-reduce-method-to-analyze-data)
...
## Solution
```js
let averageRating = watchList.filter((item) => item.Director === 'Christopher Nolan').map((item) => Number(item.imdbRating)).reduce((a, b) => a + b) / watchList.filter((item) => item.Director === 'Christopher Nolan').length;
