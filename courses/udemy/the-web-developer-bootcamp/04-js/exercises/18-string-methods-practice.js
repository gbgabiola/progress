/**
 * I've provided you with a variable in index.js called message.
 * Without altering the original value stored in message, please define a variable called whisper that is lowercased version of message, with all the extra space at the beginning and then end removed.
 * You will need to use two string methods that we just covered.
 * Don't forget you can chain them together on a single line!
 */

// DON'T TOUCH THIS LINE!
const message = '    TASTE THE RAINBOW!  '; //DON'T CHANGE THIS LINE PLEASE!

// YOUR CODE BELOW THIS LINE:
const whisper = message.toLowerCase().trim();
